...
export default class Canvas extends React.Component {
  constructor(props) {
    ...
    this.source = this.document[2].match(/```js(.*)\n([^]+)```/);
  ...
  renderSource(value) {
    import('../../../app/explain/components-config.js').then(Element => {
  ...
  render() {
    ...
    return (
      <div className={`${prefixCls}-demo-warpper ${prefixCls}-${this.props.name}`}>
        <div className={`${prefixCls}-demo-source`}>
          <div className="topBar"></div>
          <div className="source" ref="source" id={this.playerId}></div>
        </div>
        <div className={`${prefixCls}-demo-meta`} style={{
          height: this.state.showBlock ? 0 : 'inherit' 
        }}>
          {
            this.description &&
